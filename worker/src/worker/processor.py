    ...
    def process_job(self, job_file: Path):
        ...
        try:
            ...
            if selections:
                ...
                all_file_rels = [sel["rel_path"] for sel in selections]
                file_progress = {f: {"status": "queued"} for f in all_file_rels}

                for idx, sel in enumerate(selections):
                    file_rel = sel["rel_path"]
                    file_progress[file_rel] = {"status": "processing"}
                    self.update_status(job_id, "processing", (idx / total_files) * 100,
                                       current_file=file_rel, logs=all_logs,
                    ...
                    def on_progress(p: dict, _file_rel=file_rel, _idx=idx):
                        file_progress[_file_rel].update({"status": "processing", **p})
                        self.update_status(job_id, "processing",
                                           ((_idx + p.get("percent", 0) / 100) / total_files) * 100,
                                           current_file=_file_rel, logs=all_logs,
                                           file_progress=file_progress)

                    def on_cmd(c: str, _file_rel=file_rel, _idx=idx):
                        file_progress[_file_rel]["cmd"] = c
                        self.update_status(job_id, "processing",
                                           (_idx / total_files) * 100,
                                           current_file=_file_rel, logs=all_logs,
                                           file_progress=file_progress)
                    ...
                    file_progress[file_rel]["done_line"] = f"{input_path} -> {output_path}"
                    file_progress[file_rel]["status"] = "done"
                    self.update_status(job_id, "processing",
                ...
                file_progress = {f: {"status": "queued"} for f in files}
            ...
            self.update_status(job_id, "completed", 100.0, logs=all_logs,
                               file_progress=file_progress if 'file_progress' in dir() else {})
